        ...
        closeSelectEvent:function(){
            ...
            this.ele.find(".inputWrap").on("click",function(event){
                ...
                if(that.ele.find(".inputWrap>i").hasClass("fa-angle-down")){
                    ...
                    that.ele.find(".mySelect-option").animate({height:"400px"},"fast","swing");
                    that.ele.find(".mySelect-option").css("display","block");
                }else{
                    that.ele.find(".inputWrap>i").removeClass("fa-angle-up").addClass("fa-angle-down");
                    that.ele.find(".mySelect-option").animate({height:"0"},"fast","swing");
                    that.ele.find(".mySelect-option").css("display","none")
                }
            });
            ...
            this.ele.find(".mySelect-option").find("div").on("click", function (event) {
                ...
                if(that.options.mult){
                    ...
                    if($(this).hasClass("selected")){
                        ...
                        that.ele.find(".inputWrap>i").removeClass("fa-angle-up").addClass("fa-angle-down");
                        that.ele.find(".mySelect-option").animate({height:"0",display:"block"},"fast","swing")
                    }
                ...
                for(var i=0;i<this.options.option.length;i++){
                    for(var j=0;j<this.result.length;j++){
                        if(this.result[j]==this.options.option[i].value){
                            this.ele.find(".inputWrap ul").append('<li><span>'+this.options.option[i].label+'</span>&nbsp;&nbsp;<i data-value="'+this.options.option[i].value+'" class="glyphicon glyphicon-remove"></i></li>')
                        }
                    }
                }
            }else{
