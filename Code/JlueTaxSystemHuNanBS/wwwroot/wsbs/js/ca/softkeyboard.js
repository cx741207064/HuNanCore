...
while (whichIt.id!=null&& Object.prototype.toString.call(whichIt.id) == "[object String]" &&whichIt.id.indexOf("softkeyboard") == -1) {
...
if (isie) {document.all.softkeyboard.style.pixelTop = ttop+thei+16;}
else{softkeyboard.style.top = ttop+thei+16;} 

//softkeyboard.style.left = tleft+100;
if (isie) {document.all.softkeyboard.style.pixelLeft = tleft+100;}
else{ softkeyboard.style.left = tleft+100;}
...
function setHead()
{
	if(isIE() && CalcTable.cells[0]!=null){
...
//使得pin码输入框获得焦点
function focusOnKeyInput(){
	//为了使得pin码输入框自动获得焦点，先删除掉usbkey的PIN码输入框，然后再new一个重新new一个添加上了再聚焦

	var usbPwInput_div = document.getElementById("usbPwInput_div");
	var usbPwInput = document.getElementById("usbPwInput");

	usbPwInput.parentNode.removeChild(usbPwInput);

	var newUsbPwInput = document.createElement('usbPwInput');
	newUsbPwInput.innerHTML='<input class="form-control" id="usbPwInput" name="usbPwInput" type="password" onkeydown="Calc.password.value=this.value"	onclick="password1=this;showkeyboard();this.readOnly=1;Calc.password.value=\'\'" onkeypress="if(event.keyCode==13) {submitByUsbPw(); return false;}">'
	usbPwInput_div.appendChild(newUsbPwInput);
	document.getElementById('usbPwInput').focus();
